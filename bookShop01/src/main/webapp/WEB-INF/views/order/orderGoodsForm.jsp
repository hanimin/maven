...
//	alert("총주문 금액:"+total_order_goods_price);
	...
	/* var p_total_order_goods_price = Integer.parseInt(p_total_order_goods_price * 0.9); */
		...
		<tbody align=center>
			...
			<tr>
				<c:forEach var="item" items="${myOrderList }">
					...
					<td>
					<h2>
					<fmt:formatNumber value="${item.goods_sales_price*0.9}" type="number" />원 (10% 할인)</h2></td>
					<td><h2>0원</h2></td>
					<td><h2>${1500 *item.order_goods_qty}원</h2></td>
					<td>
					  <h2>
					  <fmt:formatNumber value="${(item.goods_sales_price*0.9) * item.order_goods_qty}" type="number" />원</h2>
					  <input  type="hidden" id="h_each_goods_price"  name="h_each_goods_price" value="${(item.goods_sales_price*0.9) * item.order_goods_qty}" />
					</td>
			</tr>
	...
	<table width=80% class="list_view" style="background: #ccffff">
		<tbody>
			...
			<tr cellpadding=40 align=center>
				...
				<td>
					<p id="p_totalPrice">
					<fmt:formatNumber value="${total_order_price*0.9}" type="number"  />원</p> 
					<input id="h_totalPrice" type="hidden" value="${total_order_price*0.9}" />
				</td>
				<td><IMG width="25" alt=""
				...
				<td>
					<p id="p_final_totalPrice">
						<font size="15">
						<fmt:formatNumber value="${final_total_order_price * 0.9}" var="h_final_total_Price" />
						${h_final_total_Price}원 </font>
					</p> <input id="h_final_total_Price" type="hidden" value="${h_final_total_Price}" />
				</td>
			</tr>
		</tbody>
	</table>
